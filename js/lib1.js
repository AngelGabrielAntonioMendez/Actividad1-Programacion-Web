const Validador ={
    ...
    analizarPassword: (pwd) =>{
        ...
        if (pwd.length < 8) errores.push("Faltan caracteres (mínimo 8).");
        ...
        return{
            esValidad: errores.length === 0,
            detalles: errores
        };
    }
        const Auth = {
            preapararDatosLogin: (correo, password) => {
                if (!Validador.correo(correo)){
                    throw new Error("El correo no tiene un formato válido.");   
                }
                if (!Validador.password(password)){
                    throw new Error("La contraseña debe tener al menos 8 caracteres, incluir mayúsculas y minusculas, un número y un caracter especial.");
                }
                return {
                    email: correo.trim(),
                    pwd: password,
